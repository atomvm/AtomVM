...
%%
%% A path read with --files-from must name an existing file, and that is fatal:
%% the list is generated, so a name that is not there means the generator failed
%% or wrote something else, and going on would quietly extract the exports of
%% whatever directory the name happens to point at.
%%
%% Usage:
...
parse_args(["--files-from", File | Rest], #{paths := Paths} = Opts) ->
    Listed = read_list(File),
    [
        abort("~s names ~s, which is not a file", [File, Path])
     || Path <- Listed, not filelib:is_regular(Path)
    ],
    parse_args(Rest, Opts#{paths := Paths ++ Listed});
