...
echo "$tmp/not_there.beam" > "$tmp/missing.txt"
expect_failure "extract_beam_exports.erl --files-from rejects a missing file" \
    "$tools/extract_beam_exports.erl" --files-from "$tmp/missing.txt"

echo "$tmp" > "$tmp/directory.txt"
expect_failure "extract_beam_exports.erl --files-from rejects a directory" \
    "$tools/extract_beam_exports.erl" --files-from "$tmp/directory.txt"
