...
generate_function_dies_with_module(
    ...
    WSBits = Backend:word_size() * 8,
    CtxRegNum = Backend:dwarf_ctx_register(),
    Sorted = lists:sort([{Off, FN, Ar} || {Off, FN, Ar} <- Functions, Off >= 0]),
    % Pair each function with the end address (next function's offset, or CodeSize)
    Ends =
        case Sorted of
            [] -> [];
    ...
    iolist_to_binary(
        lists:zipwith(
            fun({Offset, FunctionName, Arity}, EndAddr) ->
                ...
                <<4, NameBin/binary, 0, Offset:WSBits/little, EndAddr:WSBits/little,
                    CtxParam/binary, 0>>
            end,
            Sorted,
            Ends
...
generate_opcode_dies(Opcodes, Backend) ->
    ...
    iolist_to_binary([
        named_die(2, io_lib:format("~s@~B", [opcode_name(Op), Off]), Off, WSBits)
     || {Off, Op} <- Sorted
    ]).
...
generate_label_dies(Labels, Backend) ->
    ...
    iolist_to_binary([
        named_die(3, io_lib:format("label_~B", [L]), Off, WSBits)
     || {Off, L} <- Sorted
    ]).

%% Emit a simple DIE with name + low_pc (used for opcodes and labels)
named_die(AbbrevCode, Name, Offset, WSBits) ->
    NameBin = iolist_to_binary(Name),
    <<AbbrevCode, NameBin/binary, 0, Offset:WSBits/little>>.
...
generate_symbol_table(
    ...
    OpcodeSyms = [
        {io_lib:format("~s:op_~s@~w", [ModuleName, opcode_name(Op), Off]), Off, Size, 16#10}
     || {Off, Op, Size} <- Opcodes
    ],
    LabelSyms = [
...
%% Convert opcode identifier to a printable name for DWARF symbols.
%% Accepts both binary strings (from explicit macro calls) and
%% integers (from the parse transform).
opcode_name(Name) when is_binary(Name) -> Name;
opcode_name(Num) when is_integer(Num) -> integer_to_list(Num);
opcode_name(Name) when is_atom(Name) -> atom_to_list(Name).
