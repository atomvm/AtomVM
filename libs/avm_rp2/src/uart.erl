...
%% {@link set_format/4}, {@link set_hw_flow/3}, {@link set_fifo_enabled/2},
%% {@link set_break/2}, {@link is_writable/1}, {@link tx_wait_blocking/1},
%% {@link write_blocking/2}, {@link read_blocking/2},
%% {@link is_readable/1}, {@link is_readable_within_us/2},
%% {@link putc/2}, {@link putc_raw/2}, {@link puts/2}, {@link getc/1}.
...
open(Params) ->
    ...
    TX =:= undefined andalso error({missing_required_param, tx}),
    RX =:= undefined andalso error({missing_required_param, rx}),
    ...
    case FlowControl of
        hardware when RTS =:= undefined andalso CTS =:= undefined ->
            error({missing_required_param, rts_or_cts});
        _ ->
            ok
    end,
...
%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @returns `true' if space is available in TX FIFO, `false' otherwise
%% @doc     Check if UART TX FIFO has space available.
%% @end
%%-----------------------------------------------------------------------------
-spec is_writable(Resource :: uart_resource()) -> boolean().
...
%% @param   Resource UART resource returned by `init/2'
%% @returns `ok'
%% @doc     Wait for the UART TX FIFO to be drained.
%% @end
%%-----------------------------------------------------------------------------
-spec tx_wait_blocking(Resource :: uart_resource()) -> ok.
tx_wait_blocking(_Resource) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @param   Enabled `true' to enable FIFOs (default), `false' to disable
%% @returns `ok'
%% @doc     Enable or disable the UART FIFOs.
%% @end
%%-----------------------------------------------------------------------------
-spec set_fifo_enabled(Resource :: uart_resource(), Enabled :: boolean()) -> ok.
set_fifo_enabled(_Resource, _Enabled) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @param   Enable `true' to assert break (TX held low), `false' to clear
%% @returns `ok'
%% @doc     Assert or clear a break condition on UART transmission.
%% @end
%%-----------------------------------------------------------------------------
-spec set_break(Resource :: uart_resource(), Enable :: boolean()) -> ok.
set_break(_Resource, _Enable) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @param   Char Character to send (0..255)
%% @returns `ok'
%% @doc     Write a single character with optional CR/LF conversion.
%% @end
%%-----------------------------------------------------------------------------
-spec putc(Resource :: uart_resource(), Char :: byte()) -> ok.
putc(_Resource, _Char) ->
    erlang:nif_error(undefined).
...
%% @param   Char Character to send (0..255)
%% @returns `ok'
%% @doc     Write a single character without CR/LF conversion.
%% @end
%%-----------------------------------------------------------------------------
-spec putc_raw(Resource :: uart_resource(), Char :: byte()) -> ok.
putc_raw(_Resource, _Char) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @param   String Binary string to send
%% @returns `ok'
%% @doc     Write a string with CR/LF conversion.
%% @end
%%-----------------------------------------------------------------------------
-spec puts(Resource :: uart_resource(), String :: binary()) -> ok.
puts(_Resource, _String) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource UART resource returned by `init/2'
%% @returns `{ok, Char}' where Char is 0..255
%% @doc     Read a single character, blocking until one is available.
%% @end
%%-----------------------------------------------------------------------------
-spec getc(Resource :: uart_resource()) -> {ok, byte()}.
getc(_Resource) ->
    erlang:nif_error(undefined).
