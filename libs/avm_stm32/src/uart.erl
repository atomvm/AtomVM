...
-type peripheral() :: 1..9.
-type peripheral_name() :: string() | binary().
...
%%-----------------------------------------------------------------------------
%% @param Name UART peripheral name
%% @param Opts UART configuration options
%% @returns UART handle (pid)
%% @doc Open a UART connection with the given options.
%% @end
%%-----------------------------------------------------------------------------
-spec open(Name :: peripheral_name(), Opts :: [{atom(), term()}]) -> uart().
open(Name, Opts) ->
...
%% @doc Read currently available data from the UART.
%% @end
%%-----------------------------------------------------------------------------
...
parse_opts(Opts) ->
    ...
    Peripheral = normalize_peripheral(proplists:get_value(peripheral, Opts, ?DEFAULT_PERIPHERAL)),
...
normalize_peripheral([$U, $A, $R, $T | N]) -> list_to_integer(N);
normalize_peripheral(<<"UART", N/binary>>) -> binary_to_integer(N).
