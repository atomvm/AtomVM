...
-record(state, {
    ...
    scan_receiver :: {{pid(), term()} | function() | pid()} | undefined
}).
...
handle_call(cancel_scan, _From, #state{scan_receiver = undefined} = State) ->
    {reply, ok, State};
handle_call(cancel_scan, From, #state{ref = Ref} = State) ->
    network_port ! {self(), Ref, {cancel_scan, From}},
    {noreply, State};
...
handle_info({Ref, {scan_canceled, ReplyTo, ok}}, #state{ref = Ref} = State) ->
    spawn(fun() -> scan_reply_or_callback({scan_results, {error, canceled}}, State) end),
    gen_server:reply(ReplyTo, ok),
    {noreply, State#state{scan_receiver = undefined}};
handle_info(
    {Ref, {scan_canceled, ReplyTo, {error, _}}},
    #state{ref = Ref, scan_receiver = undefined} = State
) ->
    gen_server:reply(ReplyTo, ok),
    {noreply, State};
handle_info({Ref, {scan_canceled, ReplyTo, {error, _} = Error}}, #state{ref = Ref} = State) ->
