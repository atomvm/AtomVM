...
%%-----------------------------------------------------------------------------
%% @doc The term representing a collection of annotations. It is either a `t:location/0`
...
%%-----------------------------------------------------------------------------
%% @doc Returns the term representing the annotations Anno.
%%
...
%%-----------------------------------------------------------------------------
%% @doc Returns annotations with representation Term.
%%
...
%%-----------------------------------------------------------------------------
%% @since   OTP 18.0
%% @param   Location the location of the annotations as line number or
%%          {line, column} tuple.
...
%%-----------------------------------------------------------------------------
%% @since OTP 18.0
%% @doc   Returns the end location of the text of the annotations Anno. If there is no
...
%%-----------------------------------------------------------------------------
%% @since OTP 18.0
%% @doc   Returns the filename of the annotations Anno. If there is no filename,
...
%%-----------------------------------------------------------------------------
%% @since OTP 18.0
%% @doc   Returns `true` if annotations Anno is marked as generated. The default is to
%%        return `false`.
%%
%% @end
%%-----------------------------------------------------------------------------
...
%%-----------------------------------------------------------------------------
%% @since OTP 18.0
%% @doc   Returns the text of the annotations Anno. If there is no text, `undefined` is
%%        returned.
%%
%% @end
%%-----------------------------------------------------------------------------
...
set_location(Line, L) when ?ALINE(L), ?LLINE(Line) ->
    new_location(Line);
set_location(Line, {L, Column}) when
    ?ALINE(L),
    ?ACOLUMN(Column),
    ?LLINE(Line)
->
...
set_location({L, C} = Loc, Line) when ?ALINE(Line), ?LLINE(L), ?LCOLUMN(C) ->
    new_location(Loc);
set_location({L, C} = Loc, {Line, Column}) when
    ?ALINE(Line),
    ?ACOLUMN(Column),
    ?LLINE(L),
    ?LCOLUMN(C)
