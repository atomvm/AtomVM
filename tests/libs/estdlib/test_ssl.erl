...
test_connect_close() ->
    {ok, SSLSocket} = ssl:connect("www.atomvm.org", 443, [{verify, verify_none}, {active, false}]),
    ok = ssl:close(SSLSocket).

test_connect_error() ->
    {error, _Error} = ssl:connect("www.atomvm.org", 80, [{verify, verify_none}, {active, false}]),
    ok.

test_send_recv() ->
    {ok, SSLSocket} = ssl:connect("www.atomvm.org", 443, [
