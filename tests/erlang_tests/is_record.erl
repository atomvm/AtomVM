...
start() ->
    ...
    true = fail_with_badarg(fun() -> erlang:is_record(?ID(Mike), ?ID(1)) end),
    true = fail_with_badarg(fun() -> erlang:is_record(?ID([]), ?ID(person)) end),
    true = fail_with_badarg(fun() -> erlang:is_record(?ID({}), ?ID(person)) end),
    0.
...
is_person(X) when is_record(X, person) -> true;
is_person(_X) -> false.
