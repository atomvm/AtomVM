...
get_object_from_export_test_module() ->
    ...
    LoadedGetObjectCodeResult = code:get_object_code(export_test_module),
    %% Here there is a behavior difference that one day should be fixed
    %%
    %% On the BEAM, modules loaded with `code:load_binary` are not returned.
    %% Actually, the BEAM doesn't return bytes for a loaded module,
    %% it returns bytes for the found file, including invalid module files filled with garbage,
    %% when they are on the module search path.
    %%
    %% However let's test this behavior difference and let's document it,
    %% so at least we are sure that behavior stays consistent with documentation and no unintended
    %% changes are made.
    case erlang:system_info(machine) of
        "BEAM" ->
            error = LoadedGetObjectCodeResult,
            ok;
        _ ->
            {export_test_module, Bin, "export_test_module.beam"} = LoadedGetObjectCodeResult,
            ok
    end.
