...
-endif.
-export([start/0, test_executable_line/1]).

start() ->
    ok = test_executable_line(42),
    0.

% lldb -b \
%   -o "settings set plugin.jit-loader.gdb.enable on" \
%   -o "breakpoint set -n 'test_executable_line:start/0'" \
...
%   -o "c" \
%   -o "print term_to_int(ctx->x[0])" \
%   -- tests/test-erlang test_executable_line
%
% Will print:
% (avm_int_t) 42
% and
