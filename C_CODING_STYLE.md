...
**Note**: This is a stylistic preference for readability and C idiom consistency. For primitive types like `int`, modern compilers generate identical code for `i++` and `++i` when the return value is not used. The post-increment form `i++` is more commonly seen in C code and reads more naturally as "use i, then increment." This guide chooses `i++` for consistency with established C conventions.
...
bool date_time_is_valid(const DateTime *dt)
{
    return validate_date(dt->year, dt->month, dt->day);
}
```
...
Follow the "right-to-left" rule for const:
```c
// Read right-to-left: "ptr is a pointer to const char"
...
Every allocation must have a corresponding deallocation [AVMCCS-M013]:
...
**Guidelines:**
- Limit stack arrays to ~256 bytes [AVMCCS-M006]
- Only use big stack arrays in leaf functions (don't call other functions) [AVMCCS-M007]
- Never use VLAs or `alloca()` - they prevent static stack analysis [AVMCCS-M008]
- Prefer fixed-size arrays over VLAs - predictable is better than minimal [AVMCCS-M009]
...
#### Simple Initialization [AVMCCS-M010]
...
#### Stack-Friendly Resource Management [AVMCCS-M011]
...
#### Heap-Allocated Objects [AVMCCS-M012]
...
|------|-------------|---------|
| Braces | K&R variant (new line for functions/types) | `void f()\n{` |
...
| Cast spacing | Space after closing parenthesis | `(int *) ptr` not `(int *)ptr` |
...
| Scalar type aliases | `_t` suffix | `user_id_t`, `float_or_int32_t` |
...
| Array lengths | Match array name | `users_len`, `nodes_len` |
...
| No `string` suffix | Default is C string | `parse_iso8601(void)` not `parse_iso8601_string(void)` |
...
| Watch for side effects in macros | Don't use with `++` or function calls |
...
- Memory allocation size limits (AVMCCS-M006, AVMCCS-M007)
 ...
 * @post values[0] <= values[1] <= ... <= values[count-1] (for default compare)
 *
 * @code
 ...
 * @endcode
 */
void int_array_sort(int values[], size_t count, 
...
| AVMCCS-M006 | Limit stack arrays to ~256 bytes | Stack Allocation Limits |
| AVMCCS-M007 | Only use big stack arrays in leaf functions | Stack Allocation Limits |
| AVMCCS-M008 | Never use VLAs or `alloca()` - they prevent static stack analysis | Stack Allocation Limits |
| AVMCCS-M009 | Prefer fixed-size arrays over VLAs - predictable is better than minimal | Stack Allocation Limits |
| AVMCCS-M010 | Use `_init` for simple initialization | Simple Initialization |
| AVMCCS-M011 | Use `_create`/`_destroy` for stack-friendly resource management | Stack-Friendly Resource Management |
| AVMCCS-M012 | Use `_new`/`_delete` for heap-allocated objects | Heap-Allocated Objects |
| AVMCCS-M013 | Every allocation must have a corresponding deallocation | Heap Allocation |
| **Documentation and Comments** | | |
