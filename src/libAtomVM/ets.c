...
{
    struct EtsTable *ets_table = ets_get_table(&ctx->global->ets, ctx->process_id, name_or_ref, TableAccessWrite);
    if (IS_NULL_PTR(ets_table)) {
        return EtsBadAccess;
    }
...
{
    struct EtsTable *ets_table = ets_get_table(&ctx->global->ets, ctx->process_id, name_or_ref, TableAccessRead);
    if (IS_NULL_PTR(ets_table)) {
        return EtsBadAccess;
    }
