...
#define INTN_MAX_RES_LEN (INTN_MAX_IN_LEN + INTN_INT64_LEN + 1) // 1 digit for overflow
...
static inline bool intn_fits_int64(const intn_digit_t num[], size_t len)
{
    return (len < INTN_INT64_LEN)
        || ((len == INTN_INT64_LEN) && ((num[INTN_INT64_LEN - 1] & (uint32_t) 0x80000000) == 0));
}
