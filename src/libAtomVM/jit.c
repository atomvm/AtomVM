    ...
    uintptr_t load_address, size_t *new_elf_size)
{
    ...
    if (ehdr->e_shoff + (size_t) ehdr->e_shnum * sizeof(Elf_Shdr) > original_elf_size) {
        fprintf(stderr, "ERROR: ELF section headers extend past end of data\n");
        return NULL;
    }
    if (ehdr->e_shstrndx >= ehdr->e_shnum) {
        fprintf(stderr, "ERROR: ELF shstrndx out of range\n");
        return NULL;
    }
    ...
    if (shdrs[ehdr->e_shstrndx].sh_offset + shdrs[ehdr->e_shstrndx].sh_size > original_elf_size) {
        fprintf(stderr, "ERROR: ELF section name string table extends past end of data\n");
        return NULL;
    }

    const char *shstrtab = (const char *) (original_elf_data + shdrs[ehdr->e_shstrndx].sh_offset);
    size_t shstrtab_max = shdrs[ehdr->e_shstrndx].sh_size;

    // Find .symtab, .strtab, and .debug_* sections
    for (int i = 0; i < ehdr->e_shnum; i++) {
        // Validate section name offset
        if (shdrs[i].sh_name >= shstrtab_max) {
            continue;
        }
        // Validate section data bounds
        if (shdrs[i].sh_type != 0 && shdrs[i].sh_offset + shdrs[i].sh_size > original_elf_size) {
            continue;
        }
    ...
    TRACE("Found DWARF sections: .debug_info=%zu .debug_line=%zu .debug_abbrev=%zu .debug_str=%zu\n",
        debug_info_size, debug_line_size, debug_abbrev_size, debug_str_size);
    ...
    size_t elf_size = sizeof(Elf_Ehdr) + sizeof(Elf_Phdr) + (section_count * sizeof(Elf_Shdr)) + code_size + symtab_size + strtab_size + shstrtab_size + debug_info_size + debug_line_size + debug_abbrev_size + debug_str_size;
