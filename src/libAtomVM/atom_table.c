    ...
    atom_index_t *translate_table, enum EnsureAtomsOpt opt)
{
    ...
    for (size_t i = 0; i < count; i++) {
        struct HNode *node;
        if (is_long_format) {
            ssize_t atom_len = read_encoded_len(&current_atom);
            ...
            const uint8_t *current_atom_data = current_atom + 1;
            node = get_node(table, current_atom_data, atom_len);
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (is_long_format) {
            ...
            atom_len = (size_t) read_encoded_len(&next_atom);
            atom_data = next_atom;
            next_atom += atom_len;
        } else {
            atom_len = current_atom[0];
            atom_data = current_atom + 1;
        ...
        if (translate_table[i] == ATOM_TABLE_NOT_FOUND_MARKER) {
            ...
            unsigned long hash = sdbm_hash(current_atom + 1, atom_len);
            unsigned long bucket_index = hash % table->capacity;

            translate_table[i] = insert_node(table, node_group, bucket_index, atom_data, atom_len);
            remaining_atoms--;
