...
static inline term context_set_exception_class(Context *ctx, term exception_class)
{
    return ctx->exception_class = exception_class & ~TERM_IMMED2_TAG_MASK;
}

static inline term context_set_exception_class_use_live_flag(Context *ctx, term exception_class)
{
    return ctx->exception_class
