...
int term_funprint(PrinterFun *fun, term t, const GlobalContext *global)
{
    ...
    } else if (term_is_reference_resource(t)) {
...
#error "Unsupported endianness"
                    ...
                    case TERM_TYPE_INDEX_REFERENCE: {
                        if (!term_is_external(t) && !term_is_external(other)) {
                            ...
                            if (term_is_reference_resource(other)) {
                                other_len = 4;
                            } else {
                                other_len = 2;
                            }
                            ...
                            if (node == other_node) {
                                ...
                                if (creation == other_creation) {
                                    ...
                                    } else if (term_is_reference_resource(other)) {
                                        other_len = 4;
                                    } else {
                                        other_len = 2;
                                    }
...
term term_from_resource_type_and_data(uint64_t resource_type_ptr, uint64_t resource_data_ptr, Heap *heap, GlobalContext *glb)
{
#if UINTPTR_MAX == UINT32_MAX
    if ((resource_type_ptr >> 32) == 0 && (resource_data_ptr >> 32) == 0) {
#endif
        ...
        if (resource_type && resource_was_serialized((void *) (uintptr_t) resource_data_ptr, resource_type)) {
            resource_ptr = (void *) (uintptr_t) resource_data_ptr;
