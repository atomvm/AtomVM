...
static void neg_inplace(uint32_t num[], size_t num_len, size_t *out_len)
{
    size_t i;
    uint32_t carry = 1;
    for (i = 0; i < num_len; i++) {
...
void intn_mulmns(const uint32_t u[], int m, const uint32_t v[], int n, uint32_t w[])
{
    mulmns32(u, m, v, n, w);
}
...
int divmnu16(uint16_t q[], uint16_t r[], const uint16_t u[], const uint16_t v[], int m, int n)
{
...
void intn_divmns(const uint32_t u[], int m, const uint32_t v[], int n, uint32_t q[])
{
    uint32_t u_abs[9];
    size_t m_abs;
    ...
    if (divmnu16((uint16_t *) q, NULL, (uint16_t *) u_abs, (uint16_t *) v_abs, m_abs16, n_abs16)
        != 0) {
        abort();
    }
    if (u_neg != v_neg) {
...
char *intn_to_string(const intn_digit_t *num, size_t len, int base)
{
...
#error "Unsupported endianess"
    ...
    do {
        ...
        size_t intlen = intptr_to_a_unterminated(r, end, base);
        end -= intlen;

        m16 = count16(q, m16);

        // add padding: intptr_to_a_unterminated(7, 10, ptr) -> 7,
