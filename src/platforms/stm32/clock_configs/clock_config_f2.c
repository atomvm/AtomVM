...
/**
 * System Clock Configuration for STM32F2 family.
 *
 * Configures HSE -> PLL -> SYSCLK at 120 MHz.
 *
...
void SystemClock_Config(void)
{
...
#if AVM_CLOCK_HZ == 120000000 && HSE_VALUE == 8000000U
    /* 8MHz / 8 = 1 MHz * 240 = 240 MHz VCO / 2 = 120 MHz
     * USB: 240/5 = 48 MHz */
    rcc_osc_init.PLL.PLLM = 8;
    rcc_osc_init.PLL.PLLN = 240;
    rcc_osc_init.PLL.PLLP = RCC_PLLP_DIV2;
    rcc_osc_init.PLL.PLLQ = 5;
#elif AVM_CLOCK_HZ == 120000000 && HSE_VALUE == 25000000U
    /* 25MHz / 25 = 1 MHz * 240 = 240 MHz VCO / 2 = 120 MHz
     * USB: 240/5 = 48 MHz */
    ...
    rcc_osc_init.PLL.PLLQ = 5;
#else
#error "Unsupported clock frequency / HSE combination for STM32F2"
#endif
     ...
     *   SYSCLK = 120 MHz
     *   AHB    = 120 MHz (HPRE = /1)
     *   APB1   = 30 MHz  (PPRE1 = /4, max 30 MHz)
     *   APB2   = 60 MHz  (PPRE2 = /2, max 60 MHz)
     */
