...
/* SPI2 and SPI3 are on APB1; other SPI peripherals are on APB2.
 * STM32G0 has a single APB bus (PCLK1). */
...
static term nif_spi_set_baudrate(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(status != HAL_OK)) {
        AVM_LOGE(TAG, "HAL_SPI_Init failed in set_baudrate: %d", (int) status);
        if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
...
static term nif_spi_set_format(Context *ctx, int argc, term argv[])
{
    ...
    HAL_StatusTypeDef status = HAL_SPI_Init(&rsrc_obj->handle);
    if (UNLIKELY(status != HAL_OK)) {
        AVM_LOGE(TAG, "HAL_SPI_Init failed in set_format: %d", (int) status);
        if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
        return hal_status_to_error(ctx, status);
