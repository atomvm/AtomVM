...
static term gpiodriver_set_level(Context *ctx, term cmd)
{
    ...
    term result = gpio_digital_write(ctx, gpio_pin_tuple, level);
...
static term gpiodriver_set_direction(Context *ctx, term cmd)
{
    ...
    term result = setup_gpio_pin(ctx, gpio_tuple, direction);
    if (UNLIKELY(result != OK_ATOM)) {
        port_ensure_available(ctx, TUPLE_SIZE(2));
        return port_create_error_tuple(ctx, result);
    }
...
static term gpiodriver_read(Context *ctx, term cmd)
{
    ...
    term result = gpio_digital_read(ctx, gpio_pin_tuple);
    if (UNLIKELY(!term_is_logic_level(ctx->global, result))) {
        port_ensure_available(ctx, TUPLE_SIZE(2));
        return port_create_error_tuple(ctx, result);
    }
...
static NativeHandlerResult consume_gpio_mailbox(Context *ctx)
{
    ...
    switch (cmd) {
        ...
        case GPIOInvalidCmd: {
            ...
            free(invalid_name);
            port_ensure_available(ctx, TUPLE_SIZE(2));
            ret = port_create_error_tuple(ctx, UNDEFINED_ATOM);
            break;
        }
        ...
        default: {
            ...
            free(cmd_name);
            port_ensure_available(ctx, TUPLE_SIZE(2));
            ret = port_create_error_tuple(ctx, BADMATCH_ATOM);
        }
    }
    ...
    if (UNLIKELY(memory_ensure_free_with_roots(ctx, TUPLE_SIZE(2), 1, &ret, MEMORY_CAN_SHRINK) != MEMORY_GC_OK)) {
        ...
        ret_msg = port_create_tuple2(ctx, gen_message.ref, ret);
    }
...
static term nif_gpio_digital_read(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!term_is_logic_level(ctx->global, ret))) {
        RAISE_ERROR(ret);
    }
