...
term sys_get_info(Context *ctx, term key)
{
    ...
    if (key == globalcontext_existing_term_from_atom_string(glb, ATOM_STR("\x12", "avm_free_heap_size"))) {
        return term_from_int32(sys_get_free_heap());
    }
    if (key == globalcontext_existing_term_from_atom_string(glb, ATOM_STR("\x15", "avm_minimum_free_size"))) {
