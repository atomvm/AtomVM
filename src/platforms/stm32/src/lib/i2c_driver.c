...
static term nif_i2c_init(Context *ctx, int argc, term argv[])
{
    ...
    term own_address_term = interop_kv_get_value_default(opts, own_address_str, term_from_int(0), glb);
    ...
    VALIDATE_VALUE(own_address_term, term_is_integer);
    ...
    if (UNLIKELY(clock_speed_hz == 0)) {
        AVM_LOGE(TAG, "clock_speed_hz must be > 0");
        RAISE_ERROR(BADARG_ATOM);
    }
    ...
    rsrc_obj->handle.Init.OwnAddress1 = (uint32_t) (term_to_int(own_address_term) << 1);
    ...
    if (status != HAL_OK) {
        ...
        return create_pair(ctx, ERROR_ATOM, globalcontext_make_atom(glb, ATOM_STR("\x8", "i2c_init")));
    }
...
static term nif_i2c_master_transmit(Context *ctx, int argc, term argv[])
{
    ...
    /* HAL expects 8-bit address (7-bit address left-shifted by 1) */
...
static bool get_mem_add_size(term mem_add_size_term, uint16_t *out)
{
...
static term nif_i2c_mem_read(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_mem_add_size(argv[3], &mem_addr_size))) {
        RAISE_ERROR(BADARG_ATOM);
    }
...
static term nif_i2c_slave_transmit(Context *ctx, int argc, term argv[])
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Slave_Transmit(&rsrc_obj->handle, (uint8_t *) data, (uint16_t) len, timeout_ms);
    if (UNLIKELY(status != HAL_OK)) {
        if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
...
static term nif_i2c_slave_receive(Context *ctx, int argc, term argv[])
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Slave_Receive(&rsrc_obj->handle, buf, (uint16_t) count, timeout_ms);
    if (UNLIKELY(status != HAL_OK)) {
        if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
