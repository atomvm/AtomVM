...
static term nif_spi_set_format(Context *ctx, int argc, term argv[])
{
    ...
    spi_cpol_t cpol = (spi_cpol_t) term_to_int(argv[2]);
    spi_cpha_t cpha = (spi_cpha_t) term_to_int(argv[3]);

    spi_set_format(rsrc_obj->spi_inst, data_bits, cpol, cpha, SPI_MSB_FIRST);
...
static term nif_spi_read_blocking(Context *ctx, int argc, term argv[])
{
    ...
    spi_read_blocking(rsrc_obj->spi_inst, repeated_tx_data, buf, (size_t) count);
...
static term nif_spi_write_read_blocking(Context *ctx, int argc, term argv[])
{
    ...
    spi_write_read_blocking(rsrc_obj->spi_inst, src, dst, len);
