...
static term start_sta(term sta_config, GlobalContext *global)
{
    ...
    if (term_is_invalid_term(hostname_term)) {
        ...
        driver_data->hostname = strdup(buf);
        // Newlib implementation of strdup might return NULL
        // See: https://www.sourceware.org/git/?p=newlib-cygwin.git;a=blob;f=newlib/libc/string/strdup_r.c;h=14c80f73f18c1b91454adadc626e415f05141da5;hb=HEAD
        if (IS_NULL_PTR(driver_data->hostname)) {
            free(ssid);
            free(psk);
            free(buf);
...
static term start_ap(term ap_config, GlobalContext *global)
{
    ...
    if (term_is_invalid_term(hostname_term)) {
        ...
        driver_data->ap_hostname = strdup(buf);
        // Newlib implementation of strdup might return NULL
        if (IS_NULL_PTR(driver_data->ap_hostname)) {
            free(ssid);
            free(psk);
