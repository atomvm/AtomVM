    ...
    switch (msg->op) {
        ...
        case op_REMOVE: {
            ...
            MUTABLE_LIST_FOR_EACH (item, tmp, fds) {
                struct FDListElement *elt = GET_LIST_ENTRY(item, struct FDListElement, head);
                if (elt->fd == msg->fd) {
                    TRACE("Removing fd=%i\n", elt->fd);
                    list_remove(item);
                    free(item);
...
static bool maybe_select(struct ListHead *fds)
{
    if (!list_is_empty(fds)) {
        ...
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = CONFIG_SELECT_TASK_TIMEOUT_MS * 1000;
...
void select_task(void *args)
{
    ...
    int64_t prev_ms = current_ms();
    uint32_t prev_sleep_ms = CONFIG_SELECT_TASK_MIN_SLEEP_MS;
    double decay_factor = (CONFIG_SELECT_TASK_MAX_SLEEP_MS - CONFIG_SELECT_TASK_MIN_SLEEP_MS) / (double) CONFIG_SELECT_TASK_DECAY_MS;

    for (;;) {
        struct SelectTaskMessage msg;
        BaseType_t res = xQueueReceive(select_queue, &msg, 0);
        bool fds_updated = false;
        if (res == pdPASS) {
            update_fds(&fds, &msg);
            fds_updated = true;
#ifdef ENABLE_TRACE
        ...
        bool at_least_one_set = maybe_select(&fds);

        int64_t curr_ms = current_ms();
        uint32_t sleep_ms = CONFIG_SELECT_TASK_MIN_SLEEP_MS;
        // sleep decay linearly if there have been no changes to fds or nothing has happened on any fds
        if (!fds_updated && !at_least_one_set && curr_ms > prev_ms) {
            uint32_t delta = (uint32_t) ceil((curr_ms - prev_ms) * decay_factor);
            uint32_t new_sleep_ms = prev_sleep_ms + delta;
            sleep_ms = MIN(CONFIG_SELECT_TASK_MAX_SLEEP_MS, new_sleep_ms);
        }
...
{
    select_queue = xQueueCreate(CONFIG_SELECT_QUEUE_SIZE, sizeof(struct SelectTaskMessage));
    if (IS_NULL_PTR(select_queue)) {
        ESP_LOGE(TAG, "Failed to initialize select_queue.");
        AVM_ABORT();
    } else {
        BaseType_t res = xTaskCreate(select_task, "select_task", CONFIG_SELECT_TASK_STACK_SIZE, select_queue, 1, NULL);
        if (res != pdPASS) {
...
REGISTER_PORT_DRIVER(otp_socket, otp_socket_init, NULL, otp_socket_create_port)
#endif
