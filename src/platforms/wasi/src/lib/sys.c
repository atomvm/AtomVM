...
#if defined(__wasip2__) || defined(AVM_NO_SMP)
static void sleep_ms(int timeout_ms)
{
    if (timeout_ms == 0) {
        return;
    }
    // A negative timeout waits forever: with a single thread and nothing to
    // poll, nothing can make a process ready again.
    if (timeout_ms < 0) {
        while (true) {
...
static void sys_poll_events_with_poll(GlobalContext *glb, int timeout_ms)
{
    ...
    if (poll_count == 0) {
        // wasi:io/poll traps on an empty list
        sleep_ms(timeout_ms);
        return;
    }
    ...
    GlobalContext *global, const char *path, struct AVMPackData **avm_data)
{
    ...
    in_memory_avm->base.data = (const uint8_t *) data;

    *avm_data = &in_memory_avm->base;
    return AVM_OPEN_OK;
}
